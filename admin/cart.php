...
// 获取当前执行动作：add(添加)、update(修改)、search(查找), delete(删除)
	...
	if ($action == "add") { /* 保存购物车到数据库 */
		// 获取添加到购物的用户名
		$emailTel = $_GET["emailTel"];
		$name = $_GET["name"];
		$price = $_GET["price"];
		$amount = $_GET["amount"];
		// 构建SQL语句
		$sql = "INSERT INTO product VALUES(NULL, '$emailTel', '$name', '$price', '$amount')";
		// 执行SQL语句
		...
		$emailTel = $_GET["emailTel"];
		$amount = $_GET["amount"];
		$sql = "UPDATE product SET amount=$amount WHERE emailTel='$emailTel'";
		// 执行SQL语句
		$result = mysql_query($sql, $conn);
		if ($result)
			echo '{"status":1, "message":"success"}';
